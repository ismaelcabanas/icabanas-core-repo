...
public abstract class AbstractGenericManager<Id extends Serializable, Dto extends BaseDto<Id>, En extends Entidad<Id>> 
	...
	public Dto registrar(Dto dto) throws RegistrarException {
		...
		// compruebo el par�emtro de entrada
		if(dto == null){
		...
		try{
			...
			logger.error("Dto no v�lido: " + dto);
	...
	public Dto actualizar(Dto dto) throws ModificarException,
		...
		// si existe la entidad la modifico, en caso contrario lanzo una excepci�n
		if(entidad != null){
			try {
	...
	public void paginar(Pagina<Dto> pagina) throws PaginacionException {
		...
		try{
			...
			throw new PaginacionException("Error en la paginaci�n.", ex);
		}
	...
	public boolean validar(Dto dto) throws ValidacionException {
		...
		if(dto == null){
			throw new IllegalArgumentException("El par�metro de entrada no puede ser nulo.");
		}
		
		En entidad = adaptarToEntidad(dto);
		
		boolean resultado = entidad.valida();
		logger.debug("El resultado de la validaci�n ha sido " + resultado);
	 ...
	 * 		Si se produce un error de validaci�n al crear la entidad o la entidad exist�a con anterioridad.
	 */
	 ...
	 * 		El Dto.
	 * @throws ValidacionException
	 * 		Si se produce un error de validaci�n al actualizar la entidad
	 */
